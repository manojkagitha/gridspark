...
const Resources = () => (
  <div className="min-h-screen flex flex-col relative overflow-hidden">
    ...
    <div className="absolute top-0 left-0 w-full h-full overflow-hidden -z-10">
      <div className="absolute top-[-10%] right-[20%] w-[300px] sm:w-[400px] md:w-[500px] h-[300px] sm:h-[400px] md:h-[500px] bg-blue-600/10 rounded-full blur-[120px] animate-pulse-glow" />
    </div>

    <section className="pt-20 sm:pt-24 pb-12 sm:pb-16 md:pb-20 flex-shrink-0 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6 md:gap-8">
          {resources.map(({ type, title, description, link }, idx) => (
            <a
              href={link}
              key={title}
              className="glass-panel p-5 sm:p-6 md:p-8 rounded-2xl border border-[var(--color-border)] hover:border-[var(--color-primary)] hover:shadow-[0_0_30px_rgba(59,130,246,0.2)] hover:-translate-y-1 transition-all duration-300 group flex flex-col touch-manipulation"
              data-aos="fade-up"
              data-aos-delay={idx * 100}
            >
              <span className="text-xs sm:text-sm font-bold mb-2 sm:mb-3 text-[var(--color-secondary)] uppercase tracking-wider">
                {type}
              </span>
